...
/**
 * Nav links — customize to match your app's routes.
 */
const navLinks = [
  { label: "Home", href: "/" },
          ...
          {mounted && !isLoading ? (
            isAuthenticated ? (
              <>
                <Link
                  href="/login"
                  className="text-[15px] text-gray-700 hover:text-primary font-medium transition-colors"
                >
                  My Account
                </Link>
                <button
            ...
            <div className="flex items-center justify-between pt-5 mt-3 border-t border-gray-100">
              {isAuthenticated ? (
                ...
                <Link href="/login" onClick={closeMobileMenu}>
                  <Button size="sm" className="h-10 px-6 rounded-full text-[14px] font-semibold">
                    Login
                  </Button>
                </Link>
              )}
